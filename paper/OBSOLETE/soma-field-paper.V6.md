...
artificial neural network constructed since McCulloch and Pitts (1943) — from perceptrons
...
since McCulloch and Pitts (1943). The cortex has been modelled many times; the limbic
system has not.
...
# Appendix D: Perturbative Expansion and Feynman Diagrams

This appendix develops the perturbative sector of the Soma-Field Model — the regime in
which emotional modes undergo small fluctuations within an attractor basin. The perturbative
expansion organises these fluctuations as a sum over Feynman diagrams, each representing a
specific interaction pathway between emotional modes. The non-perturbative sector —
threshold crossings and basin transitions — is treated separately as instanton events.

## D.1 The Path Integral and MSR Action

The stochastic Langevin dynamics

$$\dot{\mathbf{e}}(t) = -\nabla H(\mathbf{e}(t)) + \eta(t)$$

can be reformulated as a path integral using the Martin-Siggia-Rose (MSR) formalism.
One introduces a conjugate response field $\tilde{\mathbf{e}}(t)$ and writes:

$$Z = \int D\mathbf{e}\, D\tilde{\mathbf{e}}\; \exp\bigl(-S[\mathbf{e},
\tilde{\mathbf{e}}]\bigr)$$

with the MSR action:

$$S = \int dt\; \tilde{e}_i\Bigl[\dot{e}_i + \nabla_i H(\mathbf{e})\Bigr]
  - \frac{\sigma^2}{2}\,\tilde{e}_i^2$$

Substituting $\nabla_i H = -\sum_j W_{ij} e_j - \theta_i$:

$$S = \int dt\;\tilde{e}_i\Bigl[\dot{e}_i - \sum_j W_{ij} e_j -
\theta_i\Bigr] - \frac{\sigma^2}{2}\,\tilde{e}_i^2$$

The quadratic part determines the propagator; higher-order terms (from the memory kernel
and threshold nonlinearity) give the interaction vertices.

## D.2 The Free Propagator
...
$\tilde{G}^R_i(\omega) = (-i\omega - W_{ii})^{-1}$.
...
```
  ...
  e_i(t') ──────────────────────────> e_i(t)
           ...
           G^R_i(t-t') = Th(t-t') exp(W_ii(t-t'))

           Stable mode (W_ii < 0): decays at rate |W_ii|.
           Unstable mode (W_ii > 0): amplifies until nonlinearity halts it.
```

## D.3 The Coupling Vertex
...
```
  ...
  Excitatory (W_ij > 0): j amplifies i.
  Inhibitory (W_ij < 0): j suppresses i.
  Asymmetric W_ij =/= W_ji: the interaction is directional.
```

At first perturbative order, the response of mode $i$ to mode $j$ is:

$$e_i(t) \;\supset\; W_{ij} \int_{-\infty}^{t} G^R_i(t - t')\, e_j(t')\, dt'$$

## D.4 Memory Kernel Vertices

The trauma memory kernel introduces a temporally non-local vertex: mode $j$ at time $t'$
drives mode $i$ at time $t > t'$ with weight $K(t - t')$:
...
```
  ...
  K(t) = sum_k  A_k exp(-t/tau_k):
  each term is a secondary propagator with its own decay rate 1/tau_k.
  Multiple kernel terms = multiple memory channels.
```

The memory kernel $K(\tau)$ is itself a sum of propagators. Each traumatic memory channel
$(A_k, \tau_k)$ is a secondary Green's function: past activations re-enter present
dynamics with characteristic timescales $\tau_k$.

## D.5 Feedback Loops and Renormalisation
...
```
  ...
  Loop integral: integral dw  G^R_fear(w) W_fs G^R_shame(w) W_sf

  Physical consequence: renormalises the effective self-coupling of FEAR
  at the loop's characteristic timescale Delta_t ~ 1/(|W_ff| + |W_ss|).
```

Each time the loop cycles, the effective coupling $W_{ij}$ is corrected upward. Repeated
co-activation of fear and shame consolidates the coupling — the Feynman diagram account
of emotional sensitisation and the development of a conditioned response.
...
Threshold crossings — transitions between attractor basins — are invisible to the
perturbative expansion. No finite sum of Feynman diagrams describes a topological change
of basin. These events are *instantons*: saddle-point solutions of the Wick-rotated action
connecting two classical minima.
...
|---|---|---|
| Propagator $G^R_i(t)$ | Green's function of free mode | How a mode evolves between interactions |
...
The perturbative sector describes the texture within a basin — day-to-day coupling,
sensitisation, habituation. The non-perturbative sector describes the pivots. A complete
account of emotional dynamics requires both.
