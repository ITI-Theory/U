...
Assessment of attractor topology — the structure of a client's emotional landscape — could in principle be done systematically using the framework's concepts. The depth and width of trauma wells, the richness of healthy attractor basins, the coupling between the somatic and cognitive registers — each is a clinical parameter that could be estimated from careful assessment and that predicts what approaches will be most effective.
