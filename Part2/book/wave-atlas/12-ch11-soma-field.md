# Scale 6 — The Soma Field / CEMI ($10^{-1}$ m)

\begin{tcolorbox}[colback=gray!8, colframe=gray!40,
  title=\textbf{Scale 6 — Brain / CEMI Field ($10^{-1}$ m)}]

\textbf{The equation (always):}
$$({\nabla}^2 + k^2)\, G(x, x') = \delta(x - x')$$

\begin{tabular}{ll}
\textbf{$k$ at this scale:} & $k = \omega/c_\text{neural} \approx 2\pi \times 40$ Hz / $6$ m/s $\approx 40$ m$^{-1}$ \\
\textbf{Physical substrate:} & Brain; 86 billion neurons; cortical layers; fascia \\
\textbf{Propagator $G$:} & CEMI field (McFadden 2002): macroscopic EMF \\
\textbf{Mind matrix rank $N$:} & $N \approx 10^{14}$ (synaptic connections) \\
\textbf{Boundary conditions:} & Cortical surface; skull as near-field boundary \\
\textbf{Consciousness:} & Emerges when $\phi_{\text{CEMI}} \geq T_c = \sqrt{2}$ (normalised) \\
\end{tabular}
\end{tcolorbox}
